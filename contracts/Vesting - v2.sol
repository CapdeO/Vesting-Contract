    ...
    UUPSUpgradeable
{
    ...
    function setInvestment(address _investor, uint256 _amount) external onlyRole(DEFAULT_ADMIN_ROLE) onlyExisting {
        uint8 phaseNumber = getCurrentPhaseNumber();
        Phase storage currentPhase = phases[phaseNumber];

        require(_amount <= currentPhase.balance, "There are not enough tokens to buy.");

        Investor storage investor = investors[_investor][phaseNumber];

        require(investor.balance + _amount <= currentPhase.maxTokensPerInvestor,"The purchase of tokens exceeds the maximum limit per user.");
    
        investor.balance += _amount;
        investor.total += _amount;
        currentPhase.balance -= _amount;
        uint256 stableAmount = (_amount * currentPhase.tokenPrice) / 1 ether;
        currentPhase.accumulatedCapital += stableAmount;
    }
